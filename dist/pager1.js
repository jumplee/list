...
var pager=function(opts){
	...
	var opts_default={
			...
			render:function(vo,view){
				var data={};
				data[this.voName]=vo;
				view.html(self.options.template(data));
			}
		};
    //获取参数
...
pager.prototype={
		...
		,init:function(){
			...
			//初始化过渡效果
			'<div class="ytable_zzc"><img class="ytable_zzc_img" src="'+opts.loading+'"></div>';
			self.viewDom.html(innerHtml+'</div>');
			...
			function fetch(callback){
				...
				$(".ytable_zzc").css({
				 ...
				 $(".ytable_zzc_img").css({
				  "top":(mask_height-30)/2,
				  "left":(mask_width-30)/2
				 });
...
//todo 应该有理想的修改地址和参数的方法。dept.jsp需要
	...
	,setCurrentPage:function(page_num){
